...
import com.flying.cattle.me.data.out.PushDepthJob;
...
public class TestController {
	 ...
	 * 返回类型 @throws
	 */
	@GetMapping("/addNewOrders2/{size}")
	public Map<String, List<Depth>> addNewOrders2(@PathVariable("size") long size) {
		try {
			CountDownLatch latch = new CountDownLatch(1);
			for (long i = 1; i <= size; i++) {
				OrderParam param = new OrderParam(Boolean.TRUE, new BigDecimal(1), BigDecimal.ONE, BigDecimal.ONE,"XBIT-USDT", Boolean.FALSE);
				MatchOrder order = paramToOrder(param);
				order.setId(SnowflakeIdWorker.generateId());
				order.setUid(i);
				if (order.getUid().longValue() == 1) {
		...
		List<Depth> list1= pushDepthJob.getMarketDepth("XBIT-USDT", true);
		List<Depth> list2= pushDepthJob.getMarketDepth("XBIT-USDT", false);
